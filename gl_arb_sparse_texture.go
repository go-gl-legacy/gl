...
func TexPageCommitment(target GLenum, level int, xoffset int, yoffset int, zoffset int,
	...
	C.glTexPageCommitmentARB(C.GLenum(target), C.GLint(level), C.GLint(xoffset),
		...
		C.GLsizei(depth), glBool(commit))
}

func TexturePageCommitment(texture uint, target GLenum, level int, xoffset int,
	yoffset int, zoffset int, width uint, height uint, depth uint, commit bool) {
	C.glTexturePageCommitmentEXT(C.GLuint(texture), C.GLenum(target), C.GLint(level),
		C.GLint(xoffset), C.GLint(yoffset), C.GLint(zoffset), C.GLsizei(width),
		C.GLsizei(height), C.GLsizei(depth), glBool(commit))
}
